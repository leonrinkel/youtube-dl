    ...
    def _real_extract(self, url):
        ...
        metadata = self._parse_json(                                            
            self._html_search_regex(                                            
                r'window\.__reflectData\s*=\s*([^;]+)', webpage, 'metadata', fatal=False) or '', 
            video_id)                                                           
                                                                                
        video_info = try_get(metadata, lambda x: x['collectionMedia'][0], dict) or {}
        video_info_id = video_info.get('id')                                    
        if video_info_id is None:
            raise ExtractorError('Unable to extract API ID')

        media_data = self._download_json(
            'https://api.gopro.com/media/%s/download' % video_info_id, video_id)

        formats = []
        for fmt in try_get(media_data, lambda x: x['_embedded']['variations'], list) or []:
            format_url = url_or_none(try_get(fmt, lambda x: x['url']))
            if not format_url:
                continue
            formats.append({
                'url': format_url,
        ...
        if title:
            title = title.replace('\n', ' ') or 'Untitled GoPro video %s' % video_id
        ...
        return {
            ...
            'timestamp': unified_timestamp(
                metadata['collection'].get('created_at')),
            'uploader_id': str_or_none(
                metadata.get('account', {}).get('nickname')),
